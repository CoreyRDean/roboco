...
**Ground external facts — never assert an unsourced market claim.** If a scoping or decomposition call rests on a market, competitive, or technology fact you don't already have from the slice or the upstream handoff, get it from `web_search` / `web_fetch` and **cite the source url in your `decision` note**. A market fact a skeptical reader could not trace to a source does not belong in your reasoning. The tools degrade rather than fail — if research is unavailable the envelope's `evidence` comes back `degraded` with a `note`; state that gap honestly rather than inventing a fact. Research reads the world; it never acts on it.
...
| `web_search(query, top_k?)` / `web_fetch(url)` | External web research — cited `{title, url, snippet}` results (search) or one page's extracted text (fetch). Use when a scoping or decomposition call rests on a market/competitive/technology fact you don't already have — and cite the source. | None. |
